...
"""
LowPBR_SmallStock.py
LowPBR + 소형주 전략의 BackTesting
"""
...
def calc_yield(**args):
    ...
    test_year = "2010"
    test_month = "11"
    PBR_upper_threshold = 5
    PBR_lower_threshold = 0.3
    num_stock_limit = 30
    ...
    PBR_upper_threshold = args['PBR_upper_threshold']
    PBR_lower_threshold = args['PBR_lower_threshold']
    ...
    df = df[(df['PBR'] < PBR_upper_threshold) & (df['PBR'] > PBR_lower_threshold)]
    df = df.sort_values(by='PBR', ascending=True)
    df = df[['PBR']]
    code_list = df.index.to_list()
    lowPBR_smallstock_list = []
    ...
    for code in code_list:
        if code in smallstock_list:
            lowPBR_smallstock_list.append(code)
            PBR_list.append(df.loc[code]['PBR'])

    """
    lowPBR_smallstock_list: The Basket
    PBR_list: PBR 값
    name_list: 종목명
    buy_price: 매수 가격
    sell_price: 매도 가격
    ...
    매도 조건: duration 기간 말
    """
    lowPBR_smallstock_list = lowPBR_smallstock_list[:num_stock_limit]
    PBR_list = PBR_list[:num_stock_limit]
    name_list = []
    buy_price = []
    sell_price = []
    yield_list = []

    for code in lowPBR_smallstock_list:
    ...
    수익률 계산
    """
    for i, code in enumerate(lowPBR_smallstock_list):
    ...
    print("* Low PBR 소형주 전략")
    print("* 기간: ", test_date + ' ~ ' + test_end_date)
    print("* 종목: ", lowPBR_smallstock_list)
    print("* 종목명: ", name_list)
    print("* PBR: ", PBR_list)
...
for year in range(start_year, end_year + 1):
    ...
    params['PBR_upper_threshold'] = PBR_upper_threshold
    params['PBR_lower_threshold'] = PBR_lower_threshold
...
file_name = './results_PBR_' + str(PBR_lower_threshold) + '_' + str(upper_yield_limit) + '_' + str(lower_yield_limit) + '.xlsx'
