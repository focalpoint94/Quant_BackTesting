...
def supertrend(fromDate, toDate, code, isStock=True, Period=10, Multiplier=3):
    """
    ...
    p1.text(df.날짜[-50], 0.9 * df.종가.min(), plot_text, style='italic',
...
def supertrend_backTester(fromDate, toDate, code_list, isStock=True, Period=10, Multiplier=3):
    df_code_list = []
    name_list = []
    profit_list = []
    ...
    for i in range(len(code_list)):
        code, name, profit, profit_with_tax, profit_without_strat, MDD_strat, MDD_without_strat = \
            supertrend(fromDate, toDate, code_list[i], isStock, Period, Multiplier)
    ...
    data = {'code': df_code_list, 'name': name_list, '수익률': profit_list, '수익률 (세금 O)': profit_with_tax_list,
            '종목 수익률': profit_without_strat_list, '전략 MDD': MDD_strat_list, '종목 MDD': MDD_without_strat_list}
...
code_list = list(temp.index[:50])

supertrend_backTester(fromDate, toDate, code_list, isStock, Period, Multiplier)
